...
class AirportFinderViewModel: ObservableObject {
    ...
    @Published var airportsData: AirportsData?
    ...
    func getListOfAirports(completion:@escaping() -> Void) {
        ...
        if let tokenContent = tokenContent {
            networkManger.getListOfAirportsFor(lat: lat,
                                               long: long,
                                               radius: radius,
                                               pageLimit: 20,
                                               pageOffset: pageOffset,
                                               sort: sort,
                                               tokenContent: tokenContent) { [unowned self] airportsData in
            ...
            getToken { [unowned self] in
                networkManger.getListOfAirportsFor(lat: lat,
                                                   ...
                                                   pageOffset: pageOffset,
                                                   sort: sort,
                                                   tokenContent: tokenContent!) { airportsData in
                    guard let airportsData = airportsData else {
                        return
                    }
                    self.airportsData = airportsData
